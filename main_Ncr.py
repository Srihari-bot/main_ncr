...
def generate_consolidated_ncr_OpenClose_excel(combined_result, report_title="NCR"):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Define watercolor-style colors for each tower (consistent with previous updates)
        tower_colors = {
            "Veridia-Tower 03": '#A3CFFA',  # Blue
            "Veridia-Tower 04": '#F5C3C2',  # Pink (matches the image for Tower 04)
        ...
        for site, color in tower_colors.items():
            tower_formats[site] = {
                ...
                'site': workbook.add_format({
                    'align': 'left', 'valign': 'vcenter', 'border': 1, 'fg_color': '#FFFFFF'  # White for module rows
                }),
                'cell': workbook.add_format({
                    'align': 'center', 'valign': 'vcenter', 'border': 1, 'text_wrap': True, 'fg_color': '#FFFFFF'  # White for module cells
                })
            }
        
        # Default formats for sites not in tower_colors
        ...
        year = now.strftime("%Y")
        date_part = f"{day}_{month_name}_{year}"  # e.g., "22_May_2025"
        ...
        standard_sites = [
            "Veridia-Club", "Veridia-Tower 03", "Veridia-Tower 04", "Veridia-Tower 05",
        ...
        tower_levels = ['M8', 'M7', 'M6', 'M5', 'M4', 'M3', 'M2', 'M1']  # Added M1
        row = 3
        site_totals = {}
        ...
        for site in sorted_sites:
            ...
            # Skip tower-specific CommonAreas, but include the standalone Common_Area
            if "CommonArea" in site and site != "Common_Area":
                continue
                
            # Select formats based on the site
            ...
            # For towers, calculate module counts
            if "Tower" in site and site != "Common_Area":
                ...
                if original_resolved_key and original_resolved_key in resolved_sites:
                    ...
                    for i, discipline in enumerate(disciplines):
                        ...
                        for module in module_list:
                            if module in resolved_module_counts:
                ...
                if original_open_key and original_open_key in open_sites:
                    ...
                    for i, discipline in enumerate(disciplines):
                        ...
                        cat = 'Finishing' if discipline == 'FW' else 'Works' if discipline == 'SW' else 'MEP'
                        for module in module_list:
                            if module in open_module_counts:
                                open_module_counts[module][cat] += 1
                
                for level in tower_levels:
                    for cat in categories:
                        resolved_counts[cat] += resolved_module_counts[level][cat]
                        open_counts[cat] += open_module_counts[level][cat]
            
            # For non-tower sites like Common_Area, calculate counts directly
            if site != "Common_Area" and "Tower" not in site:
                resolved_counts = {'Finishing': 0, 'Works': 0, 'MEP': 0}
                open_counts = {'Finishing': 0, 'Works': 0, 'MEP': 0}
                if original_resolved_key and original_resolved_key in resolved_sites:
            ...
            # For Common_Area, calculate counts directly
            if site == "Common_Area":
                resolved_counts = {'Finishing': 0, 'Works': 0, 'MEP': 0}
                open_counts = {'Finishing': 0, 'Works': 0, 'MEP': 0}
                if original_resolved_key and original_resolved_key in resolved_sites:
            ...
            # Initialize common counts for towers
            common_counts = {'Finishing': 0, 'Works': 0, 'MEP': 0}
            common_resolved = {'Finishing': 0, 'Works': 0, 'MEP': 0}
            common_open = {'Finishing': 0, 'Works': 0, 'MEP': 0}
            ...
            if "Tower" in site:
                ...
                open_common = open_sites.get(common_site_key, {})  # Include open NCRs for tower-specific CommonArea
                
                for display_cat, possible_keys in category_map.items():
                    for key in possible_keys:
                        value = resolved_common.get(key, 0)
                        if value > 0:
                            common_resolved[display_cat] += value
                        value = open_common.get(key, 0)
                        if value > 0:
                            common_open[display_cat] += value
                
                # Add common counts to tower totals
                for cat in categories:
                    common_counts[cat] = common_resolved[cat] + common_open[cat]
                    resolved_counts[cat] += common_resolved[cat]
                    open_counts[cat] += common_open[cat]
            ...
            # Add detailed rows for towers (M8 to M1)
            if "Tower" in site and site != "Common_Area":
                for level in tower_levels:
        ...
        def write_detail_sheet(sheet_name, data, title):
            ...
            for site, site_data in data.items():
                ...
                for i in range(max_length):
                    detail_worksheet.write(row, 0, site, default_site_format)
...
def generate_combined_excel_report(all_reports, filename_prefix="All_Reports"):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        # Formatting (from new code for Combined NCR, reused for all)
        title_format = workbook.add_format({
            'bold': True, 'align': 'center', 'valign': 'vcenter', 'fg_color': 'yellow', 'border': 1, 'font_size': 12
        })
        header_format = workbook.add_format({
        ...
        # Formatting for Safety/Housekeeping (from original)
        description_format = workbook.add_format({
            'align': 'left', 'valign': 'vcenter', 'border': 1, 'text_wrap': True
        })
        
        # Date handling
        now = datetime.now()  # Current date: May 22, 2025
        day = now.strftime("%d")
        month_name = now.strftime("%B")
        year = now.strftime("%Y")
        date_part = f"{day}_{month_name}_{year}"  # e.g., "22_May_2025"
        ...
        # 1. Combined NCR Report (from new code)
        ...
        # 2. Safety and Housekeeping Reports (from original, unchanged)
